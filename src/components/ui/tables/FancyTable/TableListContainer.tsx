  ...
  return (
    <>
      <TableList
        ...
        // className="border-spacing-y-[10px] border-separate -mt-2"
      >
        <TableThead>
          <TableTr>
          ...
          {!loading &&
            ...
            rows.map((row, i) => (
              <TableTr key={i} className="intro-x">
                ...
                {hasStatus && (
                  <TableTd className="first:rounded-l-md last:rounded-r-md w-20 bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b] py-0 relative before:block before:w-px before:h-8 before:bg-slate-200 before:absolute before:left-0 before:inset-y-0 before:my-auto before:dark:bg-darkmode-400">
                    <div className="flex items-center justify-center">
                      <Link
                        className="flex items-center mr-5 text-primary whitespace-nowrap"
                        href={`/administration/configurations/menus/${row.id}`}
                      >
          ...
          {!loading && rows.length === 0 && (
            <tr>
              <td className="text-center py-10" colSpan={columns.length}>
                <div className="m-auto bg-teal-100 items-center text-center p-5 h-fit w-fit rounded-full">
